...
An entity that has a `languageCode` field and a name ending in `Translation`, but which no other entity points a `translations` relation at, is not treated as a translation entity. It gets a `config.customFields` entry like any other entity that declares custom fields.
:::
